...
本节的“原始 TauriTavern”指本轮优化开始前、已接入只读监视器的 Schema 1–3 运行时；“当前版本”指 `b8ca3364` 的 Schema 10 Android 实测。两者设备和世界书规模接近，但操作序列、激活内容和 cache 热度不完全相同，因此只有同口径 A/B 使用精确百分比，跨报告结果使用保守下界。
