...
Schema 11 reports additionally include:
...
- Listener, automation, formatting, history, chat-scroll, network, invoke, interaction, and health collections are bounded. Chat-scroll reports keep a separate bounded `criticalSamples` collection so anomalous writes, DOM rebuilds, lifecycle decisions, and large geometry jumps survive high-frequency streaming. Routine writes to the current `scrollHeight` and controller scheduling bookkeeping stay out of that critical collection. Reports include observed/stored/dropped counts where truncation is relevant.
